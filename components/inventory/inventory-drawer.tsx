...
const PRESET_CATEGORIES = [
  ...
  "Filters", "Fluids & Oils", "Tyres & Wheels", "Transmission", "Other",
];
  ...
  useEffect(() => {
    if (open) {
      reset(item ? {
        ...
        unit: item.unit, quantity: String(item.quantity), low_stock_threshold: String(item.low_stock_threshold),
        cost_price: item.cost_price, selling_price: item.selling_price,
        barcode: item.barcode ?? "", location: item.location ?? "", description: item.description ?? "",
      } : { name: "", part_number: "", category: "", unit: "piece", quantity: "0", low_stock_threshold: "5", cost_price: "", selling_price: "", barcode: "", location: "", description: "" });
    }
  }, [open, item, reset]);
          ...
          >
            ...
            <form className="flex-1 overflow-y-auto px-6 py-5 space-y-4">
              <div className="space-y-1.5">
                ...
                <input {...register("name")} placeholder="e.g. Brake Pad Set" className={ic(!!errors.name)} />
              ...
              <div className="grid grid-cols-3 gap-4">
                ...
                <div className="space-y-1.5">
                  <label className="text-xs font-medium text-muted-foreground">Low Stock At</label>
              ...
              <div className="space-y-1.5">
                ...
                <input {...register("location")} placeholder="e.g. Shelf A-3" className={ic(false)} />
              </div>

              <div className="space-y-1.5">
                <label className="text-xs font-medium text-muted-foreground">Description</label>
                <textarea {...register("description")} rows={2} placeholder="Optional details..." className={cn(ic(false), "h-auto resize-none py-2")} />
              </div>
            </form>
